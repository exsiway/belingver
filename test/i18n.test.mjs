...
test('English is the default and the fallback', () => {
  ...
  assert.equal(t('translate.model'), 'Model');
  setLocale('nope');
  assert.equal(getLocale(), 'en');
});
...
test('placeholders are substituted and unknown ones are left alone', () => {
  setLocale('en');
  assert.equal(t('compose.button', { lang: 'Русский' }), 'Translate thesis → Русский');
  assert.equal(t('compose.button', {}), 'Translate thesis → {lang}');
...
test('a switch changes the text and the direction; Arabic is right-to-left', () => {
  ...
  assert.notEqual(t('translate.model'), 'Model');
  setLocale('ko');
  assert.equal(localeDir(), 'ltr');
  assert.equal(t('translate.model'), ko['translate.model']);
  setLocale('en');
});
