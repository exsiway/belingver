// Every locale file parses and is a table of strings.
...
for (const file of files) {
  test(`${file} parses and is a table of strings`, async () => {
    ...
    assert.ok(keys.length > 40, `${file} has only ${keys.length} keys`);
    for (const k of keys) {
...
// when one of them does not parse or lacks a key the manifest names.
...
for (const code of manifestLocales) {
  test(`extension/_locales/${code}/messages.json parses and names the extension`, () => {
    ...
    assert.equal(table.extName.message, 'Belingver');
  });
}
